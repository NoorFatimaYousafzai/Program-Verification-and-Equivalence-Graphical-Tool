...
class ProgramVerifier:
    def __init__(self):
        ...
        self.assertions_in_code = []

        self.unrolling_depth = 0

    def postcondition_stuff(self, line):
        print("POST CONDITION: ", line) # assert(x == 4)
        stripped = line.strip()
        print("STRIPPED: ", line)
        start = stripped.find('(')
        end = stripped.rfind(')')
        if start != -1 and end != -1:
            postcondition = stripped[start+1:end].strip()
            print("POST CONDITION 1: ", postcondition)

            # Replace variables with their latest versions.
            for base_var in self.variable_versions:
                if base_var in postcondition:
                    latest_version = self.variable_versions[base_var]
                    postcondition = postcondition.replace(base_var, f"{base_var}{latest_version}")

            print("POST CONDITION 1.5: ", postcondition)
            self.assertions_in_code.append(postcondition)
            print("assertions: ", self.assertions_in_code)
        
        return self.assertions_in_code

    def extract_assert_statements(self, code_lines):
        for line in code_lines:
            stripped = line.strip()
            if stripped.startswith("assert"):
                # Extract the condition inside the assert.
                # e.g. assert(x == 4); or assert(y > 0);
                start = stripped.find('(')
                end = stripped.rfind(')')
                if start != -1 and end != -1:
                    condition = stripped[start+1:end].strip()

                    # Replace variables with their latest versions.
                    for base_var in self.variable_versions:
                        if base_var in condition:
                            latest_version = self.variable_versions[base_var]
                            condition = condition.replace(base_var, f"{base_var}{latest_version}")

                    self.assertions_in_code.append(condition)
        print("ASSERTIONS: ", self.assertions_in_code)
        return self.assertions_in_code
        ...
        if '<' in condition_line:
            ...
            if len(parts) == 2:
                ...
                if right_side.isdigit():
                    self.unrolling_depth = int(right_side)
                    return self.unrolling_depth
        ...
        if '>' in condition_line:
            ...
            if len(parts) == 2:
                ...
                if right_side.isdigit():
                    self.unrolling_depth = int(right_side)
                    return self.unrolling_depth
        self.unrolling_depth = 4

        print(self.unrolling_depth)
        return self.unrolling_depth
    ...
    def convert_into_ssa(self, code_lines):
        ...
        #self.extract_assert_statements(code_lines)
            
        return self.ssa_lines
...
class SSAToSMTCoverter:
    def __init__(self, assertions=None, variable_versions=None, unrolling_depth=None):
        ...
        self.ud = unrolling_depth
        print("---------------------------------------------ud",self.ud)

        self.assertions_in_code_smt = assertions or []
        print("SSA TO SMT: ", self.assertions_in_code_smt)

        self.verifier_variable_versions = variable_versions
    ...
    def convert_assignment_line_in_smt(self, ssa_line):
        ...
        # Handle φ14 = (j5 < n0 - i2 - 1)
        elif left_part.startswith(condition_representation):
            print("pppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppppp")
            self.new_condition_counter += 1
            cond_name = f"cond{self.new_condition_counter}"
            self.smt_variables[left_part] = cond_name

            # Convert the condition to prefix notation
            smt_condition_expr = convert_infix_to_prefix(right_part)
            print(smt_condition_expr)

            smt_line = f"(assert (= {cond_name} {smt_condition_expr}))"
            print(smt_line)
            self.smt_assertions.append(smt_line)
    ...
    def convert_assertions_to_smt(self):

        for assertion in self.assertions_in_code_smt:
            # Convert the assertion to SMT format.
            # Example: "x == 4" becomes "(assert (= x 4))"
            # Example: "y > 0" becomes "(assert (> y 0))"

            # HANDLE ASSERTIONS LIKE: for (i in range (n)):arr[i] < arr[i+1]

            print("SSA TO SMT SOLVER ASSERTIONS 1111111111111111111111: ", self.assertions_in_code_smt)
            # verifier_variable_versions = {'i': 3, 'j': 5, 'arr0': 9, 'arr1': 13, 'temp': 5, 'arr2': 8, 'arr3': 3}
            if "for" in assertion and "range" in assertion and "arr[" in assertion: # assert(for (i in range (n)):arr[i] < arr[i+1]); 
                print("IN FOR LOOP ASSERT____________________________________+++++++++++++++++++++++++")
                # From the statement, get the condition arr[i] < arr[i+1].
                if ":" in assertion:
                    condition_part = assertion.split(":") # ['for (i in range (n))','arr[i] < arr[i+1]']
                    condition_part = condition_part[1].strip().rstrip(";")  # arr[i] < arr[i+1]
                    for_i_in_range = condition_part[0].strip() # for (i in range (n))

                    # Extract array name and operator.
                    parts = condition_part.split("<") # ['arr[i]', 'arr[i+1]']
                    left_part = parts[0].strip() # arr[i]
                    right_part = parts[1].strip() # arr[i+1]

                    if "[" in left_part:
                        left_array_parts = left_part.split("[")
                        left_array_name = left_array_parts[0].strip() # arr
                        left_index = left_array_parts[1].split("]")[0].strip() # i

                    if "[" in right_part:
                        right_array_parts = right_part.split("[")
                        right_array_name = right_array_parts[0].strip() # arr
                        right_index = right_array_parts[1].split("]")[0].strip() # i + 1

                    for i in range(self.ud - 1):
                        left = f"{left_array_name}{i}"

                        if left in self.verifier_variable_versions:
                            left_version = f"{left}_{self.verifier_variable_versions[left]}"

                        right = f"{right_array_name}{i+1}"

                        if right in self.verifier_variable_versions:
                            right_version = f"{right}_{self.verifier_variable_versions[right]}"

                        self.smt_assertions.append(f"(assert (< {left_version} {right_version})")

            # Handle equality assertions.
            elif "==" in assertion:
                left, right = assertion.split("==")
                left = left.strip()
                right = right.strip()
                smt_assert = f"(assert (= {left} {right}))"
                self.smt_assertions.append(smt_assert)
            
            # Handle greater than assertions.
            elif ">" in assertion:
                left, right = assertion.split(">")
                left = left.strip()
                right = right.strip()
    ...
    def convert_ssa_to_smt(self, ssa_lines):
        ...
        self.convert_assertions_to_smt()
...
def convert_infix_to_prefix(expression):
    ...
    # Operators sorted by precedence (lower precedence later)
    # Handle longest first (e.g., ">=" before ">")
    operators = ['>=', '<=', '==', '!=', '>', '<', '+', '-', '*', '/']

    # Find the main operator
    for op in operators:
        if op in expression:
            parts = [part.strip() for part in expression.split(op)]

            # If we have exactly two parts, it's a binary op
            if len(parts) == 2:
                left = convert_infix_to_prefix(parts[0])
                right = convert_infix_to_prefix(parts[1])
                return f"({op} {left} {right})"

            # If more than 2 parts, left-associative (e.g., a - b - c -> (- (- a b) c))
            elif len(parts) > 2:
                result = convert_infix_to_prefix(parts[0])
                for i in range(1, len(parts)):
                    right = convert_infix_to_prefix(parts[i])
                    result = f"({op} {result} {right})"
                return result

    # No operator found: return as-is (number or variable)
    return expression
...
def main():
    ...
    #verifier.postcondition_stuff("assert(x == 4);")
    # verifier.postcondition_stuff("assert(for (i in range (n)):arr[i] < arr[i+1]);")
    verifier.postcondition_stuff("assert(for (i in range (n)):arr[i] < arr[i+1]);")
    ...
    print("VARIABLE VERSIONS _________________________ ", verifier.variable_versions)

    print("\n=== SMT Conversion ===")
    converter = SSAToSMTCoverter(verifier.assertions_in_code, verifier.variable_versions, verifier.unrolling_depth)
    ...
    # program_verifier.check_program_equivalence(code_lines2, code_lines1)
